...
function dropimage() {
    ...
    {
    const existedIMG = document.getElementById("ImageToAdd");
    existedIMG.style.width = "300px";
    existedIMG.style.height = "300px";
    existedIMG.style.aspectRatio = "1/1";
    existedIMG.style.objectFit = "cover";
    existedIMG.src = imageUrl;  
    }
    if (!imageUrl) {
        // If input is empty, remove the image if it exists
        if (document.getElementById("ImageToAdd")) {
...
function updateLandingPageTemplate() {
    const heading = document.getElementById("landingHeading").value.trim();
    const text = document.getElementById("landingText").value.trim();
    const imageUrl = document.getElementById("addimageInput").value.trim();
    const ctaText = document.getElementById("CTAText").value.trim();
    const ctaBehavior = document.getElementById("CTAButtonBehavior").value;
    const ctaLink = document.getElementById("ctaLinkInput")?.value.trim();
    const ctaPhone = document.getElementById("ctaPhoneInput")?.value.trim();
    const ctaWhatsapp = document.getElementById("ctaWhatsappInput")?.value.trim();

    const container = document.getElementById("generatedLandingPage");
    container.innerHTML = ""; // Clear previous template

    // Main layout container
    const layout = document.createElement("div");
    layout.style.display = "flex";
    layout.style.flexDirection = "column"; // Top to bottom
    ...
    if (ctaText) {
        ...
        switch (ctaBehavior) {
            case "default":
                if (ctaLink) button.href = ctaLink;
                break;
            case "Behavior1":
                if (ctaPhone) button.href = "tel:" + ctaPhone;
                break;
            case "Behavior2":
                if (ctaWhatsapp) button.href = "https://api.whatsapp.com/send?phone=" + ctaWhatsapp;
                break;
        }

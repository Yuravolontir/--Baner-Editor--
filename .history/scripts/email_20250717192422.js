...
function SaveEmailBanner(event) {
    ...
    const UsersEmailBanners = JSON.parse(localStorage.getItem('UsersEmailBanners'));
    ...
    UsersEmailBanners.push(newEmailBanner); // Add the new banner to the array
    localStorage.setItem('UsersEmailBanners', JSON.stringify(UsersEmailBanners)); // Save to localStorage
...
headingPicker.on('emoji', emoji => {
  ...
  updateBannerHeading(); // 👈 manually update preview
});


textPicker.on('emoji', emoji => {
  insertAtCursor(textTextarea, emoji.emoji);
  updateBannerText(); // 👈 manually update preview
});

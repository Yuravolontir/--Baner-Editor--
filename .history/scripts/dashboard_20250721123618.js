...
if (b.designstyle || b.backgroundimage) {
    const gradients = {
        "gradient1": "linear-gradient(to right, #ff7e5f, #feb47b)",
        "gradient2": "linear-gradient(135deg, #ff6b6b, #6a00f4)",
        "gradient3": "linear-gradient(to right, #0f2027, #203a43, #2c5364)"
    };

    const gradient = gradients[b.designstyle] || "";
    const bgImage = b.backgroundimage ? `url(${b.backgroundimage})` : "";

    if (gradient && bgImage) {
        page.style.background = `${gradient}, ${bgImage}`;
    } else if (gradient) {
        page.style.background = gradient;
    } else if (bgImage) {
        page.style.background = bgImage;
    }
    ...
    page.style.backgroundColor = b.backgroundcolor === "#000000" ? "white" : b.backgroundcolor;
}

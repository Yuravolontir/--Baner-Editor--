...
function dropimage() {
        if (isTemplateRendering) return; // 🚫 Prevent recursion

    const imageUrl = document.getElementById("addimageInput").value;
    const div = document.getElementById("generatedLandingPage");

    // 🚫 Avoid recursive calls
    if (["template1", "template2", "template3"].includes(document.getElementById("templates").value)) {
        // Just insert image manually here
        const img = document.createElement("img");
        img.id = "ImageToAdd";
        img.src = imageUrl;
        ...
        const existing = document.getElementById("ImageToAdd");
        if (existing) existing.remove();
        if (imageUrl) div.appendChild(img);

        return; // ✅ skip recursive updateLandingPageTemplate
    }

    // 🧹 Clear previous image
    const existingImage = document.getElementById("ImageToAdd");
...
function updateLandingPageTemplate() {
    isTemplateRendering = true; // 🚫 prevent dropimage from recalling this
    ...
    switch (selectedTemplate) {
        case "template1":
            div.style.background = "linear-gradient(135deg, #d0cdb9ff, #cdc007ff)";
            h1.style.color = "#222";
            pre.style.color = "#333";
            cta.style.backgroundColor = "#007bff";
            cta.style.color = "#fff";
            div.append(h1, pre, img, cta);
            break;
        case "template2":
            div.style.background = "linear-gradient(135deg, #f0bcbcff, #f00c0cff)";
            h1.style.color = "#f8f8f8";
            pre.style.color = "#d0d0d0";
            cta.style.backgroundColor = "#ff6b6b";
            cta.style.color = "#fff";
            div.append(h1, img, pre, cta);
            break;
        case "template3":
    ...
    updateCTAInput();
    isTemplateRendering = false; // ✅ allow dropimage again
}
